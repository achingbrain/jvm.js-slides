...
function registerSlideshowListener(slideId, listener) {
	if(!slideShowCallbacks[slideId]) {
		slideShowCallbacks[slideId] = [];
	}

	slideShowCallbacks[slideId].push(listener);
}
...
self.prototype = {
	...
	*/
	goto: function(which) {
		...
		var id;
		var oldSlide = this.slides[this.slide];
		var oldSlideCallbacks = oldSlide.id ? slideShowCallbacks[oldSlide.id] : null;
		...
		if(slide) { // Slide actually changed, perform any other tasks needed
			...
			if(oldSlideCallbacks) {
				for(var i = 0; i < oldSlideCallbacks.length; i++) {
					if(oldSlideCallbacks[i].onLeave) {
						oldSlideCallbacks[i].onLeave();
					}
				}
			}

			var newSlideCallbacks = slide.id ? slideShowCallbacks[slide.id] : null;

			if(newSlideCallbacks) {
				for(var i = 0; i < newSlideCallbacks.length; i++) {
					if(newSlideCallbacks[i].onEnter) {
						newSlideCallbacks[i].onEnter();
					}
				}
			}
		}
